...
## Usage
Run the bpe precrocessing for the dataset to be finetuned. After setting up the downloaded vocabulary for src and tgt sentences during the preprocessing phase by ```preprocess.py``` of OpenNMT, use ```train_from``` in ```train.py``` of OpenNMT to implement the finetuning.  
